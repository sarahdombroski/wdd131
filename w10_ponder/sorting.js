...
function compareListBackwards(a, b) {
    if (a > b) {
        return -1;
    } else if (a < b) {
        return 1;
...
const sortBackwards = simpleList.sort(compareListBackwards);
// console.log(sortBackwards);

// ACTIVITY 2

function searchList1(testList, queryString) {
    function searchCallback(string) {
        if (string.toLowerCase().includes(queryString.toLowerCase())) {
            return true;
        }
        return false;
    }
    return testList.filter(searchCallback);
}

//console.log(searchList1(simpleList, "b"));

// SARAH DOMBROSKI

// ACTIVITY 3

function searchList(testList, queryString) {
    function searchCallback(string) {
        return (
    ...
    const filteredHikes = testList.filter(searchCallback);
    const sortedHikes = filteredHikes.sort((a, b) => a.distance > b.distance);
    return sortedHikes;
}

console.log(searchList(hikes, "falls"));
